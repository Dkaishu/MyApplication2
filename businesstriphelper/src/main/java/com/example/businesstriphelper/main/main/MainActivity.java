...
public class MainActivity extends FragmentActivity implements
    ...
    private int imageViewArray[] = { R.drawable.main_tab_home_btn, R.drawable.main_tab_view_btn,R.drawable.main_tab_me_btn};
    private String textViewArray[] = { "通知", "工具","我"};
...
/*
        //获取Intent，并启动对应fragment
        Intent intent_reciver = getIntent();
        int code_fragment = intent_reciver.getExtras().getInt("fragment");

           if (code_fragment == 0){}
            else {

                TabHost.TabSpec tabSpec = mTabHost.newTabSpec(textViewArray[code_fragment])
                        .setIndicator(getTabItemView(code_fragment));
                // 将Tab按钮添加进Tab选项卡中，并绑定Fragment
                mTabHost.addTab(tabSpec, fragmentArray[code_fragment], null);
                mTabHost.setTag(code_fragment);
                mTabHost.getTabWidget().getChildAt(code_fragment)
                        .setBackgroundResource(R.drawable.main_selector_tab_background);//设置Tab被选中的时候颜色改变
...
*/
    ...
    //判断启动对应的fragment
    //private void startTargetFragmeent(int){
