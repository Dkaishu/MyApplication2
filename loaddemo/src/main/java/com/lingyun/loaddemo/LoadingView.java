...
public class LoadingView extends View{
    ...
    private void setUpInit(AttributeSet set){
        ...
        setAttributeInit(set);
        initColor();
        initLoadingLines();
    }

    private void setAttributeInit(AttributeSet set){
        TypedArray ta = getContext().obtainStyledAttributes(set,R.styleable.LoadingView);
        if(ta != null){
    ...
    private void initColor(){
        ...
        for(int i = 0;i < count;i++){
            colors[i] = (int)colorEvaluator.evaluate((float)i/count,startColor,endColor);
        }
    }
